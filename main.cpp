...
class DpProductionPlanner
{
  ...
  const std::size_t porduction_capacity;
  const std::size_t store_capacity;
  const std::size_t store_cost;
  const std::size_t constant_porduction_cost;
...
public:
  DpProductionPlanner(const std::size_t i_porduction_capacity,
                      const std::size_t i_store_capacity,
                      const std::size_t i_store_cost,
                      const std::size_t i_constant_porduction_cost,
                      const std::vector<int> &i_requests) : porduction_capacity(i_porduction_capacity),
                                                            store_capacity(i_store_capacity),
                                                            store_cost(i_store_cost),
                                                            constant_porduction_cost(i_constant_porduction_cost),
                                                            stages(init_stages(porduction_capacity, store_capacity, i_requests.size())),
                                                            requests(i_requests),
  ...
  void do_the_thing()
  {
    for (int stage_it = 0; stage_it < reversed_requests.size(); ++stage_it)
    {
      for (int state = 0; state <= store_capacity; ++state)
      {
        ...
        for (int x = 0; x <= porduction_capacity; ++x)
        {
          ...
          int production_cost = x > 0 ? constant_porduction_cost : 0;
